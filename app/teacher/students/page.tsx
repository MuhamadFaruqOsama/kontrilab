...
export default function StudentsPage() {
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold font-heading text-foreground tracking-tight">Siswa</h1>
      </div>
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-default-100">
              {mockStudents.map((student) => (
                <tr key={student.id} className="hover:bg-default-50/50 transition-colors">
                  <td className="px-6 py-4 font-medium text-foreground">{student.name}</td>
                  <td className="px-6 py-4 text-default-500">{student.class}</td>
                  <td className="px-6 py-4 text-default-500">{student.project}</td>
                  <td className="px-6 py-4 text-default-500">{student.group}</td>
                  <td className="px-6 py-4 text-center text-default-500">{student.progress}</td>
                  <td className="px-6 py-4 text-center text-default-500">{student.peerAss}</td>
                  <td className="px-6 py-4 text-center">
                    <StatusBadge status={student.status} />
                  </td>
                  <td className="px-6 py-4 text-right">
                    <Link href={`/teacher/students/${student.id}`} className="p-2 inline-flex text-default-500 hover:text-primary hover:bg-primary-50 rounded-md transition-colors" title="Lihat Detail">
