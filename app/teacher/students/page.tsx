  ...
  return (
    <div className="space-y-7">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm font-medium text-ktr-text-secondary">
            Pantau aktivitas dan progres setiap siswa di semua proyek.
          </p>
        </div>
      ...
      <div className="flex items-center gap-4 text-sm">
        ...
        {needAttention > 0 && (
          <span className="rounded-full border border-ktr-warning/25 bg-ktr-warning-bg px-3 py-1 text-xs font-semibold text-[#9a620b]">
          ...
          <table className="w-full text-left text-sm">
            <thead className="border-b border-ktr-border-light bg-ktr-surface-soft text-ktr-text-secondary">
              <tr>
                ...
                <th className="px-5 py-3 font-semibold">Upload</th>
            ...
            <tbody className="divide-y divide-ktr-border-light">
              {filtered.map((student) => (
                <tr
                  key={student.id}
                  className={`transition-colors hover:bg-ktr-surface-soft/70 ${
                    student.status === "Tidak Ada Aktivitas Terbaru" || student.status === "Perlu Perhatian"
                      ? "bg-ktr-warning-bg/30"
                      : ""
                  }`}
                >
                  <td className="px-5 py-4 font-semibold text-ktr-text-primary">{student.name}</td>
                  <td className="px-5 py-4 font-medium text-ktr-text-secondary">{student.className}</td>
                  <td className="px-5 py-4 font-medium text-ktr-text-primary">{student.activeProject}</td>
                  <td className="px-5 py-4">
                    <StatusBadge status={student.status} />
                  </td>
                  <td className="px-5 py-4">
                    <span className="flex items-center gap-1.5 font-semibold text-ktr-text-primary">
                      <HugeiconsIcon icon={UserGroupIcon} size={14} className="text-ktr-text-secondary" />
                      {student.uploads}
                    </span>
                  </td>
                  ...
                  <td className="px-5 py-4 text-right">
                    <Link
                      ...
                      className="inline-flex h-9 cursor-pointer items-center gap-1.5 whitespace-nowrap rounded-[12px] border border-ktr-border-light px-3 text-sm font-semibold text-ktr-text-primary transition-colors hover:border-ktr-border-input hover:bg-ktr-primary-light active:scale-[0.995]"
                    >
                      Lihat detail
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={8} className="px-5 py-10 text-center text-sm text-ktr-text-secondary">
